...
import { Admin, emailJsConfigured } from '../admin.js'
...
export default function AdminDashboard() {
  ...
  async function remind(u) {
    ...
    setBusyId(u.id)
    try {
      await Admin.sendReminderEmail(u.email, u.full_name, u.pct)
  ...
  async function bulkRemind() {
    ...
    for (const u of targets) {
      try { await Admin.sendReminderEmail(u.email, u.full_name, u.pct); sent++ } catch {}
    }
    setBulkBusy(false)
      ...
      <div className="wrap" style={{ maxWidth: 1100 }}>
        ...
        <div className="bg-surface2 border border-line2 rounded-xl px-4 py-3 mb-5 flex items-center gap-3 flex-wrap">
          ...
          <span className="text-[13.5px] text-muted">
            {emailJsConfigured ? 'Remind everyone (not certified, not suspended) below' : 'Find everyone (not certified, not suspended) below'}
          </span>
          <input
          ...
          />
          <span className="text-[13.5px] text-muted">
            {emailJsConfigured ? '% progress' : '% progress, then remind each from the list below'}
          </span>
          <Button size="sm" variant="ghost" className="ml-auto" onClick={bulkRemind} disabled={bulkBusy}>
            {bulkBusy ? 'Sending…' : emailJsConfigured ? 'Send bulk reminder' : 'Filter list'}
          </Button>
        </div>
          ...
          {filtered.map((u) => (
            ...
            >
              ...
              <div className="flex justify-end gap-1.5">
                <button
                  title="Send reminder email" onClick={() => remind(u)} disabled={u.suspended || busyId === u.id}
